...
**当前唯一下一步：用真实 Dota 聊天采集本机诊断样本并验收已经接入的多帧共识。** 不再靠无限增加模糊词条掩盖 OCR 错字，也不提前开发其他产品功能。

### OCR 准确率改进方针（下一阶段）

1. 已实现：用 `--ocr-diagnostics` 显式开启本机采样，保存原始区域、二值化区域、TSV 单词/置信度、最终候选和可选期望文本；默认不上传，避免聊天隐私泄露。
2. 已实现：对同一聊天行采集连续 2–3 帧，按纵向位置、说话人、编辑距离、词置信度和 Dota 候选投票；明确的高置信新增行走快速路径，单帧噪声不能覆盖已有高置信结果。
...
本轮实机只验收上述实现：逐条发送指定短句，给误识别样本填写 `expected.txt`，记录是否发布、OCR 次数、从出现到 Widget 显示的时间和鼠标体感。得到同一批样本的 Tesseract 基线之前，不进入 Windows OCR 或其他引擎替换。
