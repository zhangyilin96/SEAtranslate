...
低负载真实翻译第一版首轮实测只完成“基线 7 行”，没有识别后续新聊天。已根据用户截图修正过高变化阈值、4K 全分辨率捕获和诊断可见性，修复候选通过 34 项自动测试。**当前唯一下一步是用户运行修复版复测**：进入 Dota 后确认 `OCR #` 是否从 1 增加，以及新聊天是否被翻译。得到结果前不要继续扩展 OCR 或更换翻译架构。
