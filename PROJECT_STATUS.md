...
自动测试现为 74 项，覆盖 `back / back? / farm / need farm / gogogo / stfu / no dam / lets rs / focus pa / am missing`、单字符抖动、行消失后重现、真实重复 `back`、聊天滚动、单帧低置信错误、单帧不兼容结果、三帧多数共识和玩家名/战队标签分离；TypeScript production build 通过。真实 Dota 的准确率、额外延迟与鼠标体感仍必须用本机诊断样本验收，尚不得标记 Production Ready，也尚未决定替换 Tesseract。
