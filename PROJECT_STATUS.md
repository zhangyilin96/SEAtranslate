...
自动测试现为 78 项，覆盖 `back / back? / farm / need farm / gogogo / stfu / no dam / lets rs / focus pa / am missing`、单字符抖动、行消失后重现、真实重复 `back`、聊天滚动、单帧低置信错误、单帧不兼容结果、三帧多数共识、玩家名/战队标签分离、跨 TSV 块的同视觉行重组、正确高分行不被同帧低分噪声阻塞、可信消息后的零分场景尾词清理，以及多消息突发中夹杂不稳定候选时的逐条发布；TypeScript production build 通过。真实 Dota 的准确率、额外延迟与鼠标体感仍必须用本机诊断样本验收，尚不得标记 Production Ready，也尚未决定替换 Tesseract。

2026-08-22 首次实机复测暴露基线回归：Tesseract 在 0/1/5 行之间抖动时，共识基线会反复从 `1/3` 重新计数，导致启动后约 20 秒内发送的真实消息被最终帧整体吸收为旧聊天，Widget 因而一直无输出。修复后首个完成的 OCR 帧立即建立基线，连续 2–3 帧共识只用于其后的新聊天；新增回归测试确认首帧空基线后出现的 `gogogo` 会在第二个一致帧发布，不再被基线吞掉。

同日第二次实测仍无输出，实时日志确认 Worker 正常运行但新帧候选为 0 行。使用用户提供的 4K 真实截图按已保存区域离线复现后，确认 `SINGLE_COLUMN` 会把彩色玩家名/标签与白色消息拆成不同 TSV 块，解析器因只信任 Tesseract 行号而丢弃孤立消息；同一截图中 `back` 甚至被分块识别为 `pack`。当前修复仍使用 Tesseract，将页面布局改为 `SINGLE_BLOCK`，并按纵向基线、玩家颜色与消息起始位置重新合并跨块单词。真实截图离线对比可保留 `back` 的同行上下文；正式实机发布仍待用户复测确认。

新 EXE 的本机诊断帧随后抓到 `back?`（消息置信度 96.8、玩家名置信度 84.3），同时还混入低置信度场景噪声。此前快速路径要求同一批所有候选都达到阈值，导致正确行被垃圾行整批拖住。当前修复允许空聊天窗口中的明确新批次按行筛选快速候选：只发布高置信度或高置信 Dota 语义行，低置信邻行仍不发布并保留给共识判断；普通非 Dota 快速阈值从 93 收紧到 96，避免把 `need far` 这类高分但可疑的单帧结果直接发布。

第三次实测的真实诊断帧把同一条 `gogogo` 连续识别为 `gogoge LT`、`gogoge`、`gegoge`；其中尾词 `LT` 置信度为 0，但旧解析器仍将其拼进消息，使前两帧编辑距离超过共识阈值。修复后，同一视觉行只要存在至少一个可信单词，就丢弃低于 15 分的尾部场景碎片；孤立的零分单词仍保留给多帧层判断。这样前两帧均形成 `gogoge`，可在第二帧按 Dota 近似短指令达成共识。

第四次实测确认两个性能/发布根因。其一，Desktop 原先每次探测都经 Electron 抓整张 4K 屏幕，真实日志单次约 680–730ms；共识补帧还会先探测再全图捕获，使一轮 Capture 约 1.9–2.0 秒，并造成 FPS 数字不明显下降但帧时间可感知抖动。当前改为独立的 Win32 区域截图助手，只复制已保存的聊天矩形，不注入 Dota、不读取进程内存；本机同尺寸只读基准约 63ms（探测图）/74ms（OCR 图），失败时才回退到 Electron。共识补帧直接抓 OCR 图，恢复心跳由 7 秒放宽到 20 秒；英文首帧为空时也不再同步加载泰文 OCR，而是在第二个空帧按 20 秒节流回退。

其二，空聊天窗口中的一批候选如果只有部分行先走快速路径，旧状态会把整批其余行一并提交为旧聊天，导致“发三条只显示一条”。当前只记录已发布的可信行，保留未稳定行继续补帧；新增回归覆盖 `gogogo / we need back / we need farm first / wif→wtf` 的逐步发布且不重复。`cant back / we need back / we need farm first` 增加的是明确 Dota 语义规则，不用于掩盖 OCR 模糊错误。自动测试 78/78 与 production build 通过；真实 Dota 的端到端延迟和鼠标体感仍需新 EXE 实机确认后才能标记 PASS。
